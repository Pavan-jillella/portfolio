    ...
    <div className="min-h-screen flex items-center justify-center px-6 relative overflow-hidden">
      ...
      >
        ...
        >
          ...
          >
            {mode === "reset" ? "Reset Password" : mode === "signup" ? "Create Account" : "Welcome"}
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <AnimatePresence mode="wait">
            {(error || errorParam) && (
              <motion.div
                ...
                className="mb-6 flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-red-500/[0.06] border border-red-500/15"
              >
            ...
            {success && (
              <motion.div
                ...
                animate={{ opacity: 1, height: "auto" }}
                exit={{ opacity: 0, height: 0 }}
                className="mb-6 flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-emerald-500/[0.06] border border-emerald-500/15"
              >
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                <p className="font-body text-sm text-emerald-400/90">{success}</p>
              </motion.div>
            )}
          </AnimatePresence>

          {/* Email/Password Form */}
          <form onSubmit={handleEmailAuth} className="relative z-10 space-y-4 mb-6">
            <div>
              <label htmlFor="email" className="block font-body text-xs text-white/30 mb-1.5 ml-1">
                Email
              </label>
              <input
                id="email"
                ...
                className="w-full px-4 py-3 rounded-xl font-body text-sm text-white placeholder:text-white/20 transition-all focus:outline-none focus:ring-1 focus:ring-blue-500/30"
                style={{
            ...
            {mode !== "reset" && (
              <div>
                <label htmlFor="password" className="block font-body text-xs text-white/30 mb-1.5 ml-1">
                  Password
                </label>
                <input
                  id="password"
            ...
            {mode === "signup" && (
              <div>
                <label htmlFor="confirmPassword" className="block font-body text-xs text-white/30 mb-1.5 ml-1">
                  Confirm Password
                </label>
                <input
                  id="confirmPassword"
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full rounded-xl py-3.5 font-body text-sm font-medium text-white transition-all disabled:opacity-40 disabled:cursor-not-allowed"
              style={{
          ...
          {mode !== "reset" && (
            <div className="relative z-10 flex items-center gap-3 mb-6">
              ...
              <span className="font-body text-[10px] text-white/20 uppercase tracking-wider">or</span>
              <div className="flex-1 h-px bg-white/[0.06]" />
            </div>
          )}
          ...
          <div className="mt-6 text-center relative z-10 space-y-2">
            {mode === "login" && (
              <>
                <p className="font-body text-xs text-white/20">
                  ...
                  <button onClick={() => switchMode("signup")} className="text-blue-400/70 hover:text-blue-400 transition-colors">
                    Sign up
                  </button>
                </p>
                <p>
                  <button onClick={() => switchMode("reset")} className="font-body text-[11px] text-white/15 hover:text-white/30 transition-colors">
                    Forgot password?
                  </button>
                </p>
              </>
            ...
            {mode === "signup" && (
              <p className="font-body text-xs text-white/20">
                ...
                <button onClick={() => switchMode("login")} className="text-blue-400/70 hover:text-blue-400 transition-colors">
                  Sign in
                </button>
              </p>
            )}
        ...
        >
          ...
          <p className="font-mono text-[10px] text-white/12 uppercase tracking-[0.25em] mt-3">
